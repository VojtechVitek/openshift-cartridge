...
The existing configuration in `vertx/conf` directory will be left unchanged. If you want to update the configuration as well, you might want to copy over the files from the `vertx/usr/vert.x-$version/conf` and `vertx/versions/shared/conf` directories.
